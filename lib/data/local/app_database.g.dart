...
class _$TaskDao extends TaskDao {
  ...
  Future<List<TaskModel>> getMyTasks(bool me) async {
    return _queryAdapter.queryList('SELECT * FROM tasks where me = ?1',
        mapper: (Map<String, Object?> row) => TaskModel(
            todo: row['todo'] as String,
            id: row['id'] as int?,
            me: row['me'] == null ? null : (row['me'] as int) != 0),
  ...
  Future<void> insertTask(TaskModel taskModel) async {
    await _taskModelInsertionAdapter.insert(
        taskModel, OnConflictStrategy.replace);
  }

  @override
  Future<void> updateTask(TaskModel taskModel) async {
    await _taskModelUpdateAdapter.update(taskModel, OnConflictStrategy.replace);
  }

  @override
  Future<void> deleteTask(TaskModel taskModel) async {
    await _taskModelDeletionAdapter.delete(taskModel);
  }
}
